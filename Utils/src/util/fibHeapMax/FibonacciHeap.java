 ...
 *
 * $Id: FibonacciHeap.java 536 2007-03-20 21:57:52Z perfecthash $
 *
 * Changes
 * -------
...
public class FibonacciHeap<T>
{
	...
	public void increaseKey(FibonacciHeapNode<T> x, Comparable k)
    {
        if (k.compareTo(x.key)<0) {
            throw new IllegalArgumentException(
                "increaseKey() got lower key value");
        }
    ...
    public void delete(FibonacciHeapNode<T> x)
    {
    	...
    	System.out.println("not applicable delete");
        // make x as small as possible
        //increaseKey(x, Double.NEGATIVE_INFINITY);
     ...
     */
    public void insert(FibonacciHeapNode<T> node, Comparable key)
    {
        node.key = key;
        ...
        FibonacciHeap<T> h2)
    {
        ...
        if ((h1 != null) && (h2 != null)) {
            h.maxNode = h1.maxNode;

            if (h.maxNode != null) {
                if (h2.maxNode != null) {
                    h.maxNode.right.left = h2.maxNode.left;
                    h2.maxNode.left.right = h.maxNode.right;
                    h.maxNode.right = h2.maxNode;
                    h2.maxNode.left = h.maxNode;

                    if (h2.maxNode.key.compareTo(h1.maxNode.key)>0) {
        ...
        for (int i = 0; i < arraySize; i++) {
            if (array.get(i) != null) {
                ...
                if (maxNode != null) {
                    ...
                    array.get(i).left = maxNode;
                    array.get(i).right = maxNode.right;
                    maxNode.right = array.get(i);
                    array.get(i).right.left = array.get(i);

                    // Check if this is a new min.
                    if (array.get(i).key.compareTo(maxNode.key)>0) {
                        maxNode = array.get(i);
                    }
                } else {
                    maxNode = array.get(i);
